...
func (service JudgeButton) Handle(input provider.Input) (handled bool, err error) {
	...
	matches := findNamedMatches(pollJudgmentRegex, buttonName)
	...
	canParticipate, err := security.CanGuildParticipate(service.orm, guild)
	if !canParticipate {
...
func init() {
	...
	if err != nil {
		log.Fatalln("button.judge failed to build", err)
	}
}
